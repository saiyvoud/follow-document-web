...
import { InserUser, UpdateUser } from '../../api/user';
const AddUser = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                //update data
                const res = await InserUser({ username: user.username, email: user.email, password: user.password, phoneNumber: user.phone_number, faculty_id: user.faculty_id, office_id: user.office_id });
                console.log(res)
                if (res.status === 201) {
                    navigate("/user");
                    ToastSuccess("ສຳເລັດ")
    ...
    return (
        <div className='mt-5 pb-3 w-full bg-white rounded-lg'>
            <div className='flex justify-start items-center gap-3 p-3'>
                ...
                <span className=' text-teal-500 text-lg font-bold'> ເພີ່ມຜູ້ໃຊ້</span>
            </div>
